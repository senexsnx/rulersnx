...
![Tests](https://img.shields.io/badge/tests-157%20passing-2ecc71)
...
- `Alt + G` toggles the overlay. It is a real browser shortcut, so you can rebind it
  under `about:addons` → gear icon → `Manage Extension Shortcuts`.
- No network access, no host permissions and no data collection. RulerSNX runs on a page
  only after you click its icon or press the shortcut, and only on that tab. The overlay
...
Run `npm run build:chrome` to generate `chrome-artifacts/rulersnx-chrome-1.2.0.zip`
...
| Toggle overlay | Toolbar icon, the popup, or `Alt + G` (rebindable in `about:addons`) |
| Deselect | `Esc` |
...
The engine is not a declared content script. Clicking the icon or pressing `Alt + G` grants
...
| Permission | Why |
|---|---|
| `activeTab` | Draw the overlay on the tab you are looking at, granted per click or shortcut press and gone again when that tab navigates |
...
to any site. A background event page handles the shortcut. After activation, the injected
...
npm test       # 157 assertions, headless (including Chrome worker lifecycle)
...
npm run build:chrome # chrome-artifacts/rulersnx-chrome-<version>.zip
...
src/background.js    Event page: owns the Alt+G shortcut
